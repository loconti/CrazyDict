...
def mean_formatter(mean: dict) -> str:
    ...
    if 'mean-cat' in mean:
        ...
        res = MEANCAT_FORMAT_DELIMETER.join(mean_cat) + MEAN_FORMAT_DELIMETER + trim(mean['mean'])
    else:
        res = trim(mean['mean'])
    return res
...
class VoxEl:
    def __init__(self, word='', data=None):
        ...
        self._cat_set = {}  # is the set of all active cat
        self._note = ''
    ...
    def note(self, new_notes: list[str]):
        # no None in input
        new_notes_set = set(map(filter(bool, new_notes)))
        if not all(map(text_checker, new_notes_set)):
    ...
    @cat.setter
    def cat(self, new_categories: list[list]):
    ...
    def cat_packer(self):
        ...
        self._cat = [[ck, cc] if cc in self._cat_set else [ck, None] for ck, cc in self._cat]
        self._cat = [[ck, cc] for ck, cc in self._cat if ck is not None or cc is not None]
        self._cat += [[None, nc] for nc in new_cat]
    ...
    def copy_vox_el(self):
        ...
        res._set_cat = copy.copy(self._cat_set)
